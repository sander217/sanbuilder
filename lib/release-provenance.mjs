...
export function readGitHubCliToken(spawnImpl = spawnSync) {
  const result = spawnImpl("gh", ["auth", "token", "--hostname", "github.com"], {
    encoding: "utf8",
    env: { ...process.env, GIT_TERMINAL_PROMPT: "0" },
  });
  if (result.error || result.status !== 0) return undefined;
  const token = result.stdout?.trim();
  if (!token || token.length > 4096 || /\s/.test(token)) return undefined;
  return token;
}
...
async function assertPublishedCheckoutCommit({
  ...
  if (!exactPayload) {
    throw new Error(
      `Harness HEAD ${commit} is not published in ${repository}, or the active GitHub credential cannot read that repository.`,
    );
  }
...
export async function resolveHarnessReleaseCommit({
  ...
  cliTokenProvider = readGitHubCliToken,
  timeoutMs = 15_000,
} = {}) {
  if (!root) throw new Error("Harness plugin root is required for release provenance.");
  const resolvedToken = token || (apiBase === DEFAULT_API_BASE ? cliTokenProvider() : undefined);
  ...
  return resolveRemoteCommit({ root, criticalFiles, repository, apiBase, fetchImpl, token: resolvedToken, timeoutMs });
}
